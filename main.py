...
for i in range(100000):
    ...
    if 'token' in json_response:
        ...
        # what is available
        available = []
        if cursor % 25 != 0 and str(cursor - 1) in lst:
            available.append('l')
        if ((cursor + 1) % 25!=0) and (str(cursor + 1) in lst):
            available.append('r')
        if str(cursor + 25) in lst:
            available.append('d')
        if str(cursor - 25) in lst:
            available.append('u')
        available_alt = available[:]
        if cursor % 25 != 0 and cursor-1==last_cursor and cursor - 1 in chosen_lst:
            try:
                available.remove('l')
            except:
                pass
        if ((cursor + 1) % 25 != 0) and cursor+1==last_cursor and (cursor + 1 in chosen_lst):
            try:
                available.remove('r')
            except:
                pass
        if cursor + 25 in chosen_lst and cursor+25==last_cursor:
            try:
                available.remove('d')
            except:
                pass
        if cursor - 25 in chosen_lst and cursor-25==last_cursor:
            try:
                ...
                pass
        ui = random.choice(available if available else available_alt)
        print(available)
        current_token = json_response['token']
        last_cursor=cursor
    else:
